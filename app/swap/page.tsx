...
* 
* This is a third-party testnet deployment, not Circle
* infrastructure.
  */
  const SWAP_ROUTER =
...
* 
* Used by the router for the direct USDC <-> EURC route.
  */
...
const timer = setTimeout(async () => {
  ...
  try {
    ...
    /*
     * The actual quote is performed below using
     * the wallet's configured public client.
     *
     * We create a temporary viem public client
     * so the swap page does not depend on a
     * specific wagmi public-client configuration.
     */
    const { createPublicClient, http } =
      await import("viem");
...
try {
  ...
  /*
   * Create a direct Arc Testnet public client.
   *
   * We intentionally use an explicit RPC URL here
   * because Arc Testnet gas estimation has known
   * reliability issues.
   */
  const { createPublicClient, http } =
    await import("viem");
